...
Feature 2 replaces the no-op worker effect with structured candidate extraction. The credential-free default fake recognizes one stable local example (`I prefer a dark editor theme.`) and safely returns no candidates for unknown content; it exists to verify plumbing, not model quality. `openai-compatible` deployments configure a request model tag separately from a deployment-attested immutable provenance version. The selected local path uses `MEMOS_EXTRACTION_PROVIDER=ollama`, verifies the configured tag's full digest and `completion` capability, and requires the durable version to equal `sha256:<digest>`. Active jobs whose recorded extraction version differs from the worker identity fail permanently before any provider call. Regardless of provider, strict application code validates `memory-candidate.v1`, computes trust/sensitivity/write policy, and erases rejected or review proposal content before persistence. See the [Feature 2 implementation note](docs/implementation/feature-2.md).
